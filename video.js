...
const allVideoByArtistID = (request, response) => {
    const id = request.params.id;
    db.pool.query('SELECT * FROM video_thumbnails JOIN fields ON video_thumbnails.id = fields.id WHERE video_thumbnails.author = $1 AND video_thumbnails.copyright_infringing_content IS NOT TRUE AND fields.copyright_infringing_music IS NOT TRUE AND fields.copyright_infringing_image IS NOT TRUE ORDER BY time_added DESC', [id])
    .then((results) => {
     response.status(200).json(results.rows)
    }).catch(error => console.log("GET video by artist ID" + error));
}
...
const getAllVideos = (request, response) => {
	...
	db.pool.query('SELECT * FROM fields JOIN video_thumbnails ON fields.id = video_thumbnails.id WHERE type = $1 AND video_thumbnails.copyright_infringing_content IS NOT TRUE AND fields.copyright_infringing_music IS NOT TRUE AND fields.copyright_infringing_image IS NOT TRUE AND fields.accepted IS TRUE ORDER BY time_added DESC',
		[type])
